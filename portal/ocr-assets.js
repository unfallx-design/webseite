...
const PREFIX='/assets/ocr-v2/',base=path.join(__dirname,'..','node_modules');
const files={
...
function serve(req,res,url,headers){const a=asset(url);if(a===null)return false;if(!a){res.writeHead(404,headers);res.end();return true;}
 fs.readFile(a.file,(error,data)=>{if(error){res.writeHead(503,{...headers,'Cache-Control':'no-store'});res.end('Scanner wird vorbereitet. Bitte später erneut versuchen.');return;}
 res.writeHead(200,{...headers,'Content-Type':a.type,'Content-Length':data.length,'Cache-Control':'private, max-age=86400','CDN-Cache-Control':'no-store','Surrogate-Control':'no-store','Cross-Origin-Resource-Policy':'same-origin','Content-Security-Policy':"default-src 'none'; script-src 'self' 'wasm-unsafe-eval'; connect-src 'self'"});
