...
function createMobileIntake({tx,body,rate,ip,env,authorize,onSubmitted=async()=>{}}){
 ...
 async function upload(req,cid,a){
  ...
  return tx(async s=>{
   const c=await access(s,cid,key,a),files=(await s.list('file',cid)).filter(f=>!f.deletedAt),sha=hash(bytes),existing=files.find(f=>f.mobileAssetId===asset);
   ...
   const f={id:id(),caseId:cid,kind,name,type,size:bytes.length,sha256:sha,at:new Date().toISOString(),by:a.user.name,uploadedBy:a.user.id,uploadedByRole:a.user.role,mobileAssetId:asset,perspective,orderKind:kind==='authorization'?req.headers['x-order-kind']:null,fieldsHash:kind==='authorization'?req.headers['x-fields-hash']:null};
