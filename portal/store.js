...
function storeApi(run, dialect) {
  return {
    ...
    async blob(id,data) { if(data) { await run('INSERT INTO ux_blobs (id,content) VALUES (?,?)',[id,data]); return; } const rows=await run('SELECT content FROM ux_blobs WHERE id=?',[id]); return rows[0] ? Buffer.from(rows[0].content) : null; },
    async removeBlob(id) { await run('DELETE FROM ux_blobs WHERE id=?',[id]); }
  };
}
