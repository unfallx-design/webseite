...
function createAutoixpert({env,tx,caseAccess,audit,fetcher=fetch}){
 ...
 async function step(user,cid,data){
  ...
  try{
   ...
   if(item){const bytes=await tx(async s=>{await caseAccess(s,user,cid);const file=await s.get('file',item.id);D.assert(file&&!file.deletedAt&&file.caseId===cid&&file.sha256===item.sha256,'Die Quelldatei wurde geändert.',409);return s.blob(file.id);});D.assert(D.hash(bytes)===item.sha256,'Originaldatei konnte nicht verifiziert werden.',409);
