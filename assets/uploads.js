...
function mount(el,kind=()=> 'auto',options={}){
 ...
 const urls=new Map(),nodes=new Map(),reviewButton=el.querySelector('[data-review-all]'),addButton=el.querySelector('[data-add-more]');
 ...
 function card(x){
  ...
  if(image){li.querySelector('.upload-image-open').onclick=()=>viewer.open(x.key);li.querySelector('img').onerror=e=>{e.target.hidden=true;li.querySelector('.upload-image-fallback').hidden=false;};}
  return li;
 }
 function paint(){
  const done=batch.items.filter(x=>x.state==='done').length,photos=batch.items.filter(imageItem);
  summary.textContent=batch.items.length?batch.items.length+' Dateien · '+(batch.items.reduce((n,x)=>n+x.file.size,0)/1024/1024).toLocaleString('de-DE',{maximumFractionDigits:1})+' MB · '+done+' gespeichert':'Noch keine Dateien ausgewählt.';
  ...
  input.disabled=batch.running;addButton.disabled=batch.running;reviewButton.hidden=!photos.length;reviewButton.textContent=photos.length+' Fotos durchsehen';viewer?.refresh();
 }
 function add(files){if(batch.running)return;batch.add(files,kind());paint();el.dispatchEvent(new Event('change',{bubbles:true}));}
