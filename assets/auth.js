...
async function enterPortal(){let m;try{m=await api('/me');}catch{throw new Error('Die Anmeldung konnte auf diesem Gerät nicht gespeichert werden. Bitte Cookies für diese Website erlauben und erneut anmelden.');}const home=m.user.role==='admin'||m.user.role==='appraiser'?'/gutachter-portal':m.user.role==='partner'?'/portal':'/kundenportal';const next=new URLSearchParams(location.search).get('next')||'';const fragment=/^\/(?:portal|gutachter-portal|kundenportal)#(?:fall\/[a-f0-9-]{36}|[a-z-]+)$/.test(next)?next.slice(next.indexOf('#')):'';location.replace(home+(fragment||'#start'));}
...
async function boot(){
 ...
 if(login){
  bind('#login-form',async d=>{const r=await api('/password-login',d);await enterPortal();});
  ...
  if(token){
   ...
   bind('#exchange-form',async d=>{const r=await api('/exchange',{...d,token});await enterPortal();});
  }else{
   try{const m=await api('/me');const box=document.createElement('div');box.className='portal-alert';box.innerHTML='<strong>Du bist angemeldet.</strong><p>'+esc(m.user.name)+' · '+esc(m.user.role==='admin'?'Administration':m.user.role==='appraiser'?'Gutachter-Portal':m.user.role==='partner'?'Partnerportal':'Kundenportal')+'</p><button type="button" class="btn" id="continue-portal">Meinen Arbeitsplatz öffnen →</button>';login.before(box);box.querySelector('button').addEventListener('click',()=>enterPortal().catch(e=>message(e.message,true)));}catch{}
