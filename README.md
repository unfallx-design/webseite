...
Kontaktanfragen werden vor dem Versand dauerhaft in MySQL gespeichert. Erfolgreich hochgeladene Anhänge liegen im privaten Originalspeicher. Der öffentliche Endpunkt antwortet mit `202`, `ok: true` und `delivery: queued`, sobald der Eingang bestätigt ist. Das ist keine Bestätigung einer Posteingangszustellung. Die Administration findet Nachrichten und Anhänge unter **Kontaktanfragen**. Eine Benachrichtigung an `info@unfallx.com` enthält den Link zum geschützten Eingang, keine Originalanhänge oder vollständigen Kundendaten. Antworten an die anfragende Person erfolgen über die im Eingang hinterlegte Adresse.

Die persistente Versandliste unterscheidet `pending`, `sending`, `sent`, `failed` und `uncertain`. Eindeutige Ablehnungen erhalten höchstens vier Versuche mit wachsender Wartezeit. Unklare SMTP-Annahme wird niemals automatisch wiederholt. Eine erneute Freigabe erfordert die bewusste Bestätigung durch einen Administrator. Historische Dateien unter `data/anfragen/` bleiben unverändert und müssen bei Bedarf separat übernommen werden; neue Anfragen werden dort nicht mehr gespeichert.

`GET /health` ist ausschließlich ein Liveness-Signal des Prozesses. `GET /ready` führt eine aktuelle Datenbankabfrage mit maximal drei Sekunden Antwortzeit aus und liefert bei Fehlern 503. Es sind keine vollständigen Anbieterprüfungen. **System & Verbindungen** zeigt interne, zeitgestempelte Prüfungen; der SMTP-Verbindungstest versendet keine E-Mail. Siehe [Betrieb und Wiederherstellung](docs/operations-and-backups.md).
