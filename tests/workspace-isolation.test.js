...
test('Two dedicated workspace hosts keep routes and navigation inside the correct area',()=>{
 ...
 assert.equal(H.hostPolicy('app.unfallx.com','/mobile').redirect,H.APP_ORIGIN+'/portal');
...
test('Workspace roles, host-bound sessions, request origins and access emails are enforced',async t=>{
 ...
 try{
 let partner,admin,staff;
 await t.test('Each account can only log in to its permitted host, and cookies cannot be replayed on another host',async()=>{
  for(const [w,kind]of [['partner','admin'],['partner','appraiser'],['admin','partner']]){const r=await call(w,'/password-login',{email:addresses[kind],password});assert.equal(r.status,403);assert.equal(r.cookie,null);}
  [partner,admin,staff]=await Promise.all([login('partner','partner'),login('admin','admin'),login('admin','appraiser')]);
  for(const [w,h]of [['partner',admin],['admin',partner]])assert([401,403].includes((await call(w,'/me',undefined,h)).status));
  assert.equal((await call('partner','/admin/overview',undefined,partner)).status,403);assert.equal((await call('admin','/admin/overview',undefined,staff)).status,403);
  assert.equal((await call('partner','/cases',{plate:'CSRF'},partner,{Origin:H.MOBILE_ORIGIN})).status,403);
 ...
 await t.test('Partner cases stay visible to their partner and internal team, with tenant and version protection',async()=>{
  const created=await call('partner','/cases',{vehicle:'Partner test vehicle',plate:'TEST-10'},partner);assert.equal(created.status,200);const id=created.json.case.id;
  ...
  assert.equal((await call('partner','/cases/'+id,{...created.json.case.intake,action:'save',version:created.json.case.version},partner)).status,409);
  const other=await login('partner','other');assert.equal((await call('partner','/cases/'+id,undefined,other)).status,404);
 });
 await t.test('Retired mobile host cannot log in, mutate cases or trigger emails',async()=>{
  const before=sent.length;
  for(const [p,data] of [['/me',undefined],['/cases',undefined],['/cases',{plate:'RETIRED'}],['/password-login',{email:addresses.partner,password}],['/password/request',{email:addresses.partner}]]){
   const r=await call('retired',p,data,partner);assert.equal(r.status,410);assert.equal(r.cookie,null);assert.equal(r.json.redirect,H.APP_ORIGIN+'/login');
  }
  assert.equal(sent.length,before);assert.equal((await call('partner','/me',undefined,partner)).status,200);
 });
 await t.test('Reset and invitation links use the account’s correct domain; wrong-host tokens do not consume or mutate accounts',async()=>{
  ...
  await call('admin','/admin/invite',{name:'Invited fixture',email:'invite@fixture.example'},admin);mail=sent.findLast(m=>m.to==='invite@fixture.example');assert(mail.text.includes(H.ADMIN_ORIGIN+'/login#token='));const invitation=mail.text.match(/#token=([a-f0-9]{64})/)[1];assert.equal((await call('partner','/exchange',{token:invitation})).status,403);assert.equal((await call('admin','/exchange',{token:invitation})).status,200);
  await call('partner','/password/request',{email:addresses.partner});assert(sent.findLast(m=>m.to===addresses.partner).text.includes(H.APP_ORIGIN+'/passwort#token='));
