...
test('App access stays separate while report articles move to the public company page',()=>{
 ...
 for(const p of ['/unfallgutachten','/einsatzgebiete','/wertminderung']){assert.equal(hosts.hostPolicy('unfallx.com',p).redirect,hosts.PUBLIC_ORIGIN+'/ueber-uns#'+p.slice(1));assert.equal(hosts.hostPolicy('app.unfallx.com',p).redirect,hosts.PUBLIC_ORIGIN+'/ueber-uns#'+p.slice(1));assert.equal(hosts.hostPolicy('gutachten.unfallx.com',p).redirect,hosts.PUBLIC_ORIGIN+'/ueber-uns#'+p.slice(1));}
