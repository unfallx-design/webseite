...
test('Authenticated original uploads above 6 MB are durable, idempotent and private',async()=>{
 ...
 async function actor(role,name){const id=D.hash(name),token=D.random(),csrf=D.random();await store.transaction(async s=>{await s.put('user',{id,name,email:name+'@example.com',role,active:true,verifiedAt:new Date().toISOString()},'internal');await s.put('session',{id:D.hash(token),userId:id,csrf,createdAt:new Date().toISOString(),expires:Date.now()+3600000},id);});return {Cookie:'ux_session='+token,'X-CSRF-Token':csrf};}
 ...
 try{
 const customer=await actor('customer','fixture-customer'),other=await actor('customer','fixture-other'),admin=await actor('admin','fixture-admin');
