...
test('explicit links between public website, reports and workspaces retain their destinations',()=>{
 for(const [name,isApp,isReport,workspace] of contexts)for(const origin of [H.PUBLIC_ORIGIN,H.REPORT_ORIGIN,H.APP_ORIGIN,H.ADMIN_ORIGIN])for(const suffix of ['/','/#kontakt','/?quelle=header','/datenschutz']){const href=origin+suffix;assert.equal(H.links('<a href="'+href+'">Link</a>',isApp,true,isReport,workspace),'<a href="'+href+'">Link</a>',name+' to '+href);}
});
test('report pages open on their own domain',()=>{
 for(const [name,isApp,isReport,workspace] of contexts){for(const p of ['/unfallgutachten','/wertgutachten','/einsatzgebiete'])assert.equal(H.links('<a href="'+p+'">Link</a>',isApp,true,isReport,workspace),'<a href="'+H.REPORT_ORIGIN+p+'">Link</a>');}
});
