...
test('Customer intake remains isolated; academy confirmation, capacity and schedule use durable transactions',async t=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'ux-customer-academy-'));const env={NODE_ENV:'test',PORTAL_LOCAL_DB:path.join(dir,'test.sqlite'),PORTAL_ORIGIN:'http://localhost',PORTAL_TRUST_PROXY:'true'};const sent=[];let fail=false;const mail={ready:true,send:async(to,subject,text,attachments,html)=>{sent.push({to,subject,text,html});if(fail)throw new Error('MAIL_TEST');}};
 ...
 async function signin(address,customer=true){const r=await call(customer?'/register-customer':'/login',{email:address,name:'Beispielkunde',phone:'03000000',privacy:true,role:'admin',companyId:'evil'});assert.equal(r.status,200);const email=sent.findLast(x=>x.to===address),token=email.text.match(/#token=([a-f0-9]+)/)[1];const auth=await call('/exchange',{token});assert.equal(auth.status,200);const actor={cookie:auth.cookie.split(';')[0],csrf:''};const me=(await call('/me',undefined,actor)).json;return {...actor,csrf:me.csrf,user:me.user,redirect:auth.json.redirect};}
 try{
 ...
 assert.equal(a.user.role,'customer');assert.equal(a.user.companyId,null);assert.equal(a.redirect,'/kundenportal');assert.equal((await call('/admin/overview',undefined,a)).status,403);assert.equal((await call('/admin/academy',undefined,a)).status,403);assert.equal((await call('/admin/overview',undefined,admin)).json.team.some(u=>u.role==='customer'),false);
 const email=sent.find(x=>x.to===a.user.email);assert.match(email.text,/Schaden aufnehmen/);assert.match(email.html,/Privat|Schadenaufnahme/);
 });
 ...
 const first=await call('/cases',intake,a);assert.equal(first.status,200);const cid=first.json.case.id;assert.equal(first.json.case.ownerUserId,a.user.id);assert.equal(first.json.case.finance,undefined);
 ...
 await t.test('Customer cannot read another tenant or use staff actions/files',async()=>{
 ...
 assert.equal((await upload(a,'report',pdf,'application/pdf')).status,403);assert.equal((await upload(a,'partner_invoice',pdf,'application/pdf')).status,403);
 ...
 assert.equal((await act(a,{action:'submit'})).status,200);assert.equal((await act(a,{action:'save',...intake})).status,403);assert.equal((await upload(a,'photo',photo,'image/jpeg')).status,403);
 ...
 registration={mode:'berlin',week:'start-folgt',name:'Kurs Beispiel',email:'course@example.com',phone:'0300000',privacy:true};
