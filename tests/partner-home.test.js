...
async function render({role='partner',status='approved',hash='',failMoney=false,rows=[]}={}){
 ...
 vm.runInNewContext(fs.readFileSync(path.resolve(__dirname,'../assets/workspace-ui.js'),'utf8'),sandbox);vm.runInNewContext(source,sandbox);for(let i=0;i<8;i++)await new Promise(r=>setImmediate(r));
 return {html:find('#portal-root').innerHTML,nav:find('#portal-nav').innerHTML,mobile:find('#portal-mobile-nav').innerHTML,top:find('#portal-top-action').innerHTML,calls,redirects};
}
test('approved partner opens the overview even with the legacy case-list preference',async()=>{const r=await render();assert.match(r.html,/Dein Überblick/);assert.match(r.html,/Fall im Browser aufnehmen/g);assert.match(r.html,/37,50/);assert.match(r.html,/12,50/);assert.match(r.html,/81,00/);assert(!r.html.includes('50,00'));assert(!r.redirects.some(x=>x.includes('#faelle')));assert.match(r.nav,/#provision/);});
test('an explicit case-list deep link remains intact',async()=>{const r=await render({hash:'#faelle'});assert.match(r.html,/Meine Schäden/);assert.match(r.html,/case-search/);assert(!r.html.includes('partner-wallet'));assert(!r.calls.some(x=>x.includes('/mobile/overview')));});
...
test('commission read failure leaves unknown amounts, exposes retry and preserves intake',async()=>{const r=await render({failMoney:true});assert.match(r.html,/ws-commission-strip/);assert.match(r.html,/data-retry-partner/);assert.match(r.html,/Fall im Browser aufnehmen/g);assert(!r.html.includes('0,00'));});
...
test('capture is directly reachable from overview, primary navigation, mobile bar and header',async()=>{const r=await render();for(const html of [r.html,r.nav,r.mobile,r.top])assert.match(html,/href="#neu"/);assert(r.html.indexOf('partner-capture')<r.html.indexOf('ws-commission-strip'));assert(r.nav.indexOf('href="#neu"')<r.nav.indexOf('Weitere Funktionen'));assert.match(r.mobile,/aria-label="Schaden aufnehmen"/);assert.match(r.top,/Schaden aufnehmen/);});
