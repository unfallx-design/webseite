...
test('approved partner opens the overview even with the legacy case-list preference',async()=>{const r=await render();assert.match(r.html,/Dein Überblick/);assert.match(r.html,/Schaden aufnehmen/g);assert.match(r.html,/37,50/);assert.match(r.html,/12,50/);assert.match(r.html,/81,00/);assert(!r.html.includes('50,00'));assert(!r.redirects.some(x=>x.includes('#faelle')));assert.match(r.nav,/#verguetung/);});
...
test('capture is directly reachable from overview, primary navigation, mobile bar and header',async()=>{const r=await render();for(const html of [r.html,r.nav,r.mobile,r.top])assert.match(html,/href="#neu"/);assert(r.html.indexOf('partner-capture')<r.html.indexOf('ws-commission-strip'));assert(r.nav.indexOf('href="#neu"')<r.nav.indexOf('Profil & Firma'));assert.match(r.mobile,/aria-label="Schaden aufnehmen"/);assert.match(r.top,/Schaden aufnehmen/);});
